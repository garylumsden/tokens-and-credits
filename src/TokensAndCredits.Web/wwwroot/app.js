...
const state = {
    ...
    embeddingRelationshipResult: null,
    embeddingAnalogyResult: null,
    liveEmbeddingResult: null,
};

const el = (id) => document.getElementById(id);
const THEME_STORAGE_KEY = "tokens-and-credits-theme";
...
function applyTheme(theme, persist = true) {
    ...
    el("themeToggleIcon").textContent = theme === "dark" ? "\u2600" : "\u263E";
    el("themeToggleLabel").textContent = `${nextTheme[0].toUpperCase()}${nextTheme.slice(1)} mode`;
...
// Keep each merge readable without delaying the first visible change.
function frameHold() {
    return 1700;
}

// Render the starting state immediately, pause briefly, then advance at a steady pace.
function scheduleFrames(frames, render) {
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        render(frames[frames.length - 1]);
        return;
    }
...
async function openExplainer() {
    ...
    if (!el("explainer").classList.contains("hidden") && explainer.index === 0) {
        explainerRender();
    }
}
...
function trapExplainerFocus(event) {
    if (event.key !== "Tab" || el("explainer").classList.contains("hidden")) {
        return;
    }
    const focusable = [...el("explainer").querySelectorAll(
...
function usesEmbeddingAnalogy() {
    return embeddingDemo.terminology === "analogy";
}

function embeddingCopy(technical, analogy) {
    return usesEmbeddingAnalogy() ? analogy : technical;
}
...
function renderEmbeddingSourceSwitch() {
    ...
    const technicalButton = el("embeddingTerminologyTechnical");
    const analogyButton = el("embeddingTerminologyAnalogy");
    const usesAnalogy = usesEmbeddingAnalogy();
    technicalButton.classList.toggle("active", !usesAnalogy);
    analogyButton.classList.toggle("active", usesAnalogy);
    technicalButton.setAttribute("aria-pressed", String(!usesAnalogy));
    analogyButton.setAttribute("aria-pressed", String(usesAnalogy));

    el("embeddingTitle").textContent = usesAnalogy
        ? "How language becomes geometry"
        : "How text embeddings work";
    el("embeddingKicker").textContent = embeddingCopy(
        isStatic ? "Static GloVe word embeddings" : "Azure text embeddings",
        isStatic ? "A map analogy for static word embeddings" : "A map analogy for Azure text embeddings",
    );
    el("embeddingIntro").textContent = embeddingCopy(
        isStatic
            ? "GloVe learns one context-independent vector per vocabulary word from global word-word co-occurrence statistics. Some relationships appear as approximately parallel vector offsets."
            : "An embedding model maps each input text to a high-dimensional vector. Cosine similarity compares the direction of two embedding vectors.",
        isStatic
            ? "Analogy: imagine each word as a point on a map. Words used in similar contexts tend to occupy related regions, and some relationships follow similar directions."
            : "Analogy: imagine each complete text as a point on a map. Texts with related meanings tend to have vectors that point in similar directions.",
    );
}

function captureEmbeddingDraft() {
    if (embeddingDemo.mode === "live") {
        if (el("liveEmbeddingfirst")) {
            embeddingDemo.liveRequest = readLiveEmbeddingForm();
        }
        return;
    }

    if (el("embeddingPositiveA")) {
        embeddingDemo.request = readAnalogyForm();
    }
    if (el("embeddingRelationshipFromA")) {
        embeddingDemo.relationshipRequest = readRelationshipForm();
    }
}

function setEmbeddingMode(mode) {
    captureEmbeddingDraft();
    embeddingDemo.mode = mode;
    if (mode === "live") {
        setEmbeddingStatus(
            liveEmbeddingAvailable()
                ? ""
                : "The Azure text embedding deployment is not configured.",
            !liveEmbeddingAvailable(),
        );
    } else {
        setEmbeddingStatus("");
...
function setEmbeddingTerminology(terminology) {
    captureEmbeddingDraft();
    embeddingDemo.terminology = terminology;
    renderEmbeddingStage();
}
...
function buildContextStage(stage) {
    ...
    copy.append(
        makeNode("h3", "", embeddingCopy(
            "GloVe uses word co-occurrence statistics",
            "Words learn from the company they keep",
        )),
        makeNode(
            "p",
            "",
            embeddingCopy(
                "GloVe learns word vectors from aggregate counts of how often words occur near one another in the training corpus. Each vocabulary word has one context-independent vector.",
                "Imagine reading many short windows around each word. Words that repeatedly appear in similar company receive locations in related parts of the map.",
            ),
        ),
    );
...
function vectorPreviewFor(word, result = state.embeddingResult) {
    const previews = result?.vectorPreviews || {};
...
function buildVectorStage(stage) {
    ...
    const intro = makeNode(
        ...
        "stage-lead",
        embeddingCopy(
            `Each vocabulary word has a ${embeddingDimension()}-dimensional vector. These rows show the first six coordinates from ${embeddingDatasetLabel()}.`,
            `Each word has an address with ${embeddingDimension()} coordinates. These rows show the first six coordinates from ${embeddingDatasetLabel()}.`,
        ),
    );
    ...
    const note = makeNode(
        ...
        "stage-note",
        embeddingCopy(
            "Individual coordinates do not have predefined linguistic meanings. Comparisons use the complete vector.",
            "No single coordinate has a plain-language meaning. The complete address is what matters.",
        ),
    );
    stage.append(intro, list, note);
}

function relationshipValues(result = state.embeddingResult) {
    result ||= {};
...
function buildRelationshipDiagram(
    result = state.embeddingResult,
    request = {
        fromA: embeddingDemo.request.negative,
        toA: embeddingDemo.request.positiveB,
        fromB: embeddingDemo.request.relationFrom,
        toB: embeddingDemo.request.relationTo,
    },
) {
    const values = relationshipValues(result);
    const panel = makeNode("div", "relationship-visual");
    const canvas = makeNode("div", "relation-canvas");
    const legend = makeNode("div", "relation-legend");
    [
    ["primary", `${request.fromA} \u2192 ${request.toA}`],
    ["secondary", `${request.fromB} \u2192 ${request.toB}`],
    ].forEach(([tone, labelText]) => {
    const item = makeNode("span", "relation-legend-item");
    item.append(
        makeNode("i", `relation-legend-swatch ${tone}`, ""),
        makeNode("span", "", labelText),
    );
    ...
    const appendArrow = (tone, angleDegrees) => {
    const start = { x: 105, y: 182 };
    const length = 220;
    ...
    line.setAttribute("class", `relation-svg-line ${tone}`);
    line.setAttribute("x1", String(start.x));
    line.setAttribute("y1", String(start.y));
    line.setAttribute("x2", String(end.x));
    line.setAttribute("y2", String(end.y));
    const head = document.createElementNS("http://www.w3.org/2000/svg", "path");
    head.setAttribute("class", `relation-svg-head ${tone}`);
    head.setAttribute(
        "d",
        `M${firstPoint.x} ${firstPoint.y} L${end.x} ${end.y} L${secondPoint.x} ${secondPoint.y}`,
    );
    svg.append(line, head);
    };

    appendArrow("primary", -8);
    appendArrow("secondary", -(8 + Math.min(35, Math.max(8, values.angle))));
    const origin = document.createElementNS("http://www.w3.org/2000/svg", "circle");
    origin.setAttribute("class", "relation-svg-origin");
    origin.setAttribute("cx", "105");
    origin.setAttribute("cy", "182");
    origin.setAttribute("r", "6");
    svg.appendChild(origin);
    canvas.append(legend, svg);

    const metrics = makeNode("div", "relationship-metrics");
    const cosine = makeNode("div", "relationship-metric");
    cosine.append(
        makeNode("span", "", embeddingCopy("Cosine similarity", "Direction match (cosine)")),
        makeNode("strong", "", values.cosine.toFixed(3)),
    );
    const angle = makeNode("div", "relationship-metric");
    angle.append(
        makeNode("span", "", embeddingCopy("Equivalent angle", "Angle between directions")),
...
function buildRelationshipStage(stage) {
    ...
    heading.append(
        makeNode("h3", "", embeddingCopy(
            "Compare two vector offsets",
            "Compare two directions on the map",
        )),
        makeNode(
            "p",
            "",
            embeddingCopy(
                "Subtracting one word vector from another produces an offset vector. Cosine similarity measures the alignment between two offsets.",
                "Moving from one word address to another gives a direction. Similar relationships can point in similar directions.",
            ),
        ),
    );
    ...
    const note = makeNode(
        ...
        "stage-note",
        embeddingCopy(
            "The diagram shows the measured angle between two high-dimensional offset vectors. It is not a two-dimensional projection of the embedding space.",
            "This sketch compares two directions. It does not draw the complete word map.",
        ),
    );
    stage.appendChild(note);
}

function embeddingCandidates(result = state.embeddingResult) {
    return result?.candidates || result?.rankedCandidates || [];
}

function renderCandidateRanking(result = state.embeddingResult) {
    const panel = makeNode("div", "analogy-results");
    panel.appendChild(makeNode("h3", "", embeddingCopy(
        "Nearest neighbours by cosine similarity",
        "Words closest to the calculated location",
    )));
    const candidates = embeddingCandidates(result);
    if (candidates.length === 0) {
        panel.appendChild(makeNode("p", "stage-note", embeddingCopy(
            "Run the calculation to rank the nearest vocabulary words.",
            "Run the experiment to find the closest words on the map.",
        )));
        return panel;
    }
...
function readRelationshipForm() {
    return {
        fromA: el("embeddingRelationshipFromA").value.trim().toLowerCase(),
        toA: el("embeddingRelationshipToA").value.trim().toLowerCase(),
        fromB: el("embeddingRelationshipFromB").value.trim().toLowerCase(),
        toB: el("embeddingRelationshipToB").value.trim().toLowerCase(),
    };
}
...
function buildAnalogyStage(stage) {
    ...
    presets.appendChild(makeNode("span", "preset-label", "Examples"));
    ...
    lab.appendChild(makeNode("h3", "", embeddingCopy(
        "Compute a vector analogy",
        "Move through the word map",
    )));
    ...
    const run = makeNode(
        "button",
        "primary analogy-run",
        embeddingDemo.loading
            ? "Calculating..."
            : embeddingCopy("Calculate nearest neighbours", "Find the closest words"),
    );
    run.type = "submit";
    ...
    limitation.textContent = embeddingCopy(
        "Candidates are ranked by cosine similarity to the normalized A \u2212 B + C vector. The three input words are excluded.",
        "Start at A, move away from B, then move toward C. The app lists the closest remaining word addresses.",
    );
...
function buildBridgeStage(stage) {
    ...
    const pipelineLabels = embeddingCopy(
        ["text", "tokens", "input embeddings", "contextual hidden states", "next-token logits"],
        ["text", "word pieces", "starting coordinates", "context-shaped coordinates", "next-token choices"],
    );
    pipelineLabels.forEach((label, index, labels) => {
    ...
    if (usesEmbeddingAnalogy()) {
        staticSide.append(
            makeNode("h3", "", "A fixed word map"),
            makeNode("p", "", "Each vocabulary word has one address."),
            makeNode("p", "", "The address does not change when the surrounding text changes."),
        );
        modernSide.append(
            makeNode("h3", "", "A changing token map"),
            makeNode("p", "", "Each token starts with coordinates."),
            makeNode("p", "", "The model repeatedly updates those coordinates using the surrounding context."),
        );
    } else {
        staticSide.append(
            makeNode("h3", "", "Static word embeddings"),
            makeNode("p", "", "One context-independent vector per vocabulary word."),
            makeNode("p", "", "Vector offsets can encode some syntactic and semantic relationships."),
        );
        modernSide.append(
            makeNode("h3", "", "Transformer language models"),
            makeNode("p", "", "The model converts tokens to input embeddings."),
            makeNode("p", "", "Each transformer layer computes context-dependent hidden states for every token."),
        );
    }
    ...
    const conclusion = makeNode(
        ...
        "bridge-conclusion",
        embeddingCopy(
            "Static word embeddings and transformer hidden states are both vectors, but they are different representations. A transformer computes a hidden state for each token at each layer.",
            "Both use coordinates, but they are different maps. The static map keeps one address per word; the transformer changes each token's coordinates as it processes context.",
        ),
    );
...
function buildLiveContextStage(stage) {
    ...
    heading.append(
        makeNode("h3", "", embeddingCopy(
            "The model embeds each complete input",
            "Each complete text gets one location",
        )),
        makeNode(
            "p",
            "",
            embeddingCopy(
                "The API returns one embedding vector for each input text. It does not return a separate embedding for the highlighted word.",
                "In this map analogy, the complete text gets a location. The highlighted word does not get a separate pin.",
            ),
        ),
    );
    ...
    note.append(
        makeNode("strong", "", embeddingCopy("Expected result: ", "What to look for: ")),
        document.createTextNode(
            embeddingCopy(
                "each within-topic pair should have a higher cosine similarity than a cross-topic pair.",
                "the two financial texts should be closer to each other than to the river texts, and vice versa.",
            ),
        ),
    );
...
function buildLiveEmptyState(stage, title) {
    ...
    panel.append(
        ...
        makeNode(
            ...
            "",
            liveEmbeddingAvailable()
                ? "Run the Azure model in Vector experiment to generate the embeddings."
...
function buildLiveVectorStage(stage) {
    ...
    stage.appendChild(makeNode(
        ...
        "stage-lead",
        embeddingCopy(
            `The Azure embedding model returned a ${result.dimensions}-dimensional vector for each input text. The UI shows the first six coordinates; server-side calculations use all dimensions.`,
            `Each text received an address with ${result.dimensions} coordinates. The UI shows the first six; the complete address is used for every calculation.`,
        ),
    ));
...
function liveSimilarityCard(title, leftRole, rightRole, tone) {
    ...
    card.append(
        makeNode("h3", "", title),
        score,
        makeNode("span", "live-similarity-angle", `${values.angle.toFixed(1)}\u00B0 equivalent angle`),
        track,
    );
    return card;
}
...
function buildLiveRelationshipStage(stage) {
    ...
    heading.append(
        makeNode("h3", "", embeddingCopy(
            "Compare the embedding vectors",
            "Compare where the text vectors point",
        )),
        makeNode("p", "", embeddingCopy(
            "Cosine similarity compares vector direction. Higher values indicate greater similarity for these embeddings.",
            "Vectors that point in a similar direction receive a higher match score.",
        )),
    );
    ...
    stage.append(
        ...
        makeNode(
            ...
            "stage-note",
            embeddingCopy(
                "The API returned one embedding for each complete input text. It did not return a token-level representation for bank.",
                "Each complete text has one map location. The word bank does not have its own separate pin in this result.",
            ),
        ),
    );
}
...
function buildLiveExperimentStage(stage) {
    ...
    form.append(
        makeNode("h3", "", embeddingCopy(
            "Generate and compare Azure text embeddings",
            "Place four texts on the map",
        )),
        makeNode(
            "p",
            "stage-note",
            embeddingCopy(
                "The server sends four input texts in one embeddings request. The API returns one embedding vector per input.",
                "The model gives each complete text a location represented by a list of coordinates.",
            ),
        ),
    );
    const fields = makeNode("div", "live-text-grid");
    fields.append(
        liveTextField("first", "A", "first financial input"),
        liveTextField("second", "B", "subtract this embedding"),
        liveTextField("third", "C", "add this embedding"),
        liveTextField("target", "Target", "compare with A \u2212 B + C"),
    );
    const run = makeNode(
        "button",
        "primary live-embedding-run",
        embeddingDemo.loading ? "Calling Azure..." : "Generate embeddings",
    );
    run.type = "submit";
    ...
    if (state.liveEmbeddingResult) {
        ...
        result.append(
            makeNode("span", "live-call-badge", "Azure embeddings result"),
            ...
            makeNode(
                ...
                "stage-note",
                embeddingCopy(
                    "The server computes A \u2212 B + C from the returned embeddings, then compares that vector with Target by cosine similarity.",
                    "Start at A, move away from B, then move toward C. The score compares the resulting direction with Target.",
                ),
            ),
        );
    } else {
        result.append(
            ...
            makeNode(
                ...
                liveEmbeddingAvailable()
                    ? embeddingCopy(
                        "Generate embeddings to view pairwise cosine similarities and the vector calculation.",
                        "Generate embeddings to place the texts on the map and compare their directions.",
                    )
...
function buildLiveBridgeStage(stage) {
    const pipeline = makeNode("div", "llm-pipeline");
    const pipelineLabels = embeddingCopy(
        ["input text", "embedding model", "embedding vector", "vector index", "retrieved text"],
        ["input text", "map maker", "text location", "searchable map", "nearby text"],
    );
    pipelineLabels.forEach((label, index, labels) => {
        pipeline.appendChild(makeNode("span", "pipeline-step", label));
        if (index < labels.length - 1) {
    ...
    const comparisons = embeddingCopy(
        [
            ["Static GloVe", "One context-independent vector per vocabulary word.", "Trained from global word-word co-occurrence statistics."],
            ["Text embedding model", "One vector for each input text.", "Used for similarity, clustering, classification, and retrieval."],
            ["Generative transformer", "One hidden state per token at each layer.", "Produces next-token logits from contextual hidden states."],
        ],
        [
            ["Fixed word map", "One address for each vocabulary word.", "The address stays the same in every sentence."],
            ["Text map", "One location for each complete input.", "Nearby locations help find related text."],
            ["Changing token map", "Each token's coordinates change as context is processed.", "The final coordinates help select the next token."],
        ],
    );
    comparisons.forEach(([title, first, second]) => {
    ...
    stage.append(
        ...
        makeNode(
            ...
            "bridge-conclusion",
            embeddingCopy(
                "An embedding API does not expose the internal token hidden states of a generative model. It returns a separate embedding vector for the complete input text.",
                "These maps share the idea of coordinates, but they are not interchangeable. The embedding API returns a text location, not the transformer's changing internal token map.",
            ),
        ),
    );
}
...
const EMBEDDING_STEPS = [
    {
        key: "context",
        label: "Co-occurrence",
        liveLabel: "Input texts",
        analogyLabel: "Context",
        liveAnalogyLabel: "Text examples",
        liveCaption: "The API embeds each complete input text, not the highlighted word in isolation.",
        liveAnalogyCaption: "The complete text receives a map location, so the surrounding words affect where it lands.",
        caption: "GloVe learns word vectors from global word-word co-occurrence statistics.",
        analogyCaption: "Words used in similar contexts tend to receive addresses in related parts of the map.",
        build: buildContextStage,
    },
    {
        key: "vectors",
        label: "Word vectors",
        liveLabel: "Embedding vectors",
        analogyLabel: "Coordinates",
        liveAnalogyLabel: "Text locations",
        liveCaption: "Inspect coordinates returned by the Azure embedding deployment. Calculations use every vector dimension.",
        liveAnalogyCaption: "Inspect the coordinates that form each text's location on the map.",
        caption: "Each vocabulary word has one context-independent vector with learned coordinates.",
        analogyCaption: "Each word has one address made from many coordinates.",
        build: buildVectorStage,
    },
    {
        key: "relationships",
        label: "Vector offsets",
        liveLabel: "Cosine similarity",
        analogyLabel: "Directions",
        liveAnalogyLabel: "Direction match",
        liveCaption: "Compare embedding vectors with cosine similarity and the corresponding angle.",
        liveAnalogyCaption: "Compare how closely the text vectors point in the same direction.",
        caption: "Subtract word vectors to form offsets, then compare the offsets with cosine similarity.",
        analogyCaption: "Compare the directions between word addresses.",
        build: buildRelationshipStage,
    },
    {
        key: "analogy",
        label: "Vector analogy",
        liveLabel: "Vector experiment",
        analogyLabel: "Word-map experiment",
        liveAnalogyLabel: "Map experiment",
        liveCaption: "Send four texts to Azure, compute A \u2212 B + C from their embedding vectors, and compare the result with Target by cosine similarity.",
        liveAnalogyCaption: "Place four texts on the map, then move from A away from B and toward C before comparing with Target.",
        caption: "Compute A \u2212 B + C and rank the bundled vocabulary by cosine similarity.",
        analogyCaption: "Move from one word address through the map and find the closest remaining words.",
        build: buildAnalogyStage,
    },
    {
        key: "bridge",
        label: "Transformers",
        liveLabel: "Model comparison",
        analogyLabel: "Changing maps",
        liveAnalogyLabel: "Map comparison",
        liveCaption: "Compare static word embeddings, text embedding models, and contextual hidden states in generative transformers.",
        liveAnalogyCaption: "Compare a fixed word map, a text map, and the transformer's changing internal token map.",
        caption: "Compare static word embeddings with the contextual hidden states computed by transformer language models.",
        analogyCaption: "Compare a fixed word map with the coordinates that transformers update for every token.",
        build: buildBridgeStage,
    },
];

function embeddingResultState(isStale) {
    return makeNode(
        "span",
        `embedding-result-state${isStale ? " stale" : ""}`,
        isStale ? "Inputs changed \u2014 calculate again" : "Calculated result",
    );
}

function markEmbeddingStale(kind) {
    const property = `${kind}Stale`;
    embeddingDemo[property] = true;
    const stateNode = el(`${kind}ResultState`);
    if (stateNode) {
        stateNode.textContent = "Inputs changed \u2014 calculate again";
        stateNode.classList.add("stale");
    }
}

function wireEmbeddingDraftInputs(ids, kind) {
    ids.forEach((id) => {
        el(id)?.addEventListener("input", () => markEmbeddingStale(kind));
    });
}

function embeddingPairFields(firstId, firstLabel, firstValue, secondId, secondLabel, secondValue) {
    const row = makeNode("div", "embedding-pair-fields");
    row.append(
        inputField(firstId, firstLabel, firstValue),
        makeNode("span", "relation-arrow", "\u2192"),
        inputField(secondId, secondLabel, secondValue),
    );
    return row;
}

function buildStaticRelationshipExperiment() {
    const section = makeNode("section", "embedding-experiment");
    const form = makeNode("form", "embedding-experiment-form");
    form.append(
        makeNode("h3", "", embeddingCopy(
            "Measure relationship similarity",
            "Compare two directions",
        )),
        makeNode("p", "stage-note", embeddingCopy(
            "Compare two known vector offsets. Cosine similarity and angle express the same directional comparison.",
            "Compare how closely two word-to-word directions point the same way. The score and angle describe the same match.",
        )),
        embeddingPairFields(
            "embeddingRelationshipFromA",
            "Relationship A: from",
            embeddingDemo.relationshipRequest.fromA,
            "embeddingRelationshipToA",
            "Relationship A: to",
            embeddingDemo.relationshipRequest.toA,
        ),
        embeddingPairFields(
            "embeddingRelationshipFromB",
            "Relationship B: from",
            embeddingDemo.relationshipRequest.fromB,
            "embeddingRelationshipToB",
            "Relationship B: to",
            embeddingDemo.relationshipRequest.toB,
        ),
    );
    const run = makeNode(
        "button",
        "primary embedding-run",
        embeddingDemo.loading === "relationship"
            ? "Measuring relationship similarity\u2026"
            : embeddingCopy("Measure relationship similarity", "Compare directions"),
    );
    run.type = "submit";
    run.disabled = Boolean(embeddingDemo.loading);
    form.appendChild(run);
    form.addEventListener("submit", (event) => {
        event.preventDefault();
        runEmbeddingRelationship(readRelationshipForm());
    });

    const resultPanel = makeNode("div", "embedding-result-panel");
    const result = state.embeddingRelationshipResult;
    if (!result) {
        resultPanel.appendChild(makeNode(
            "p",
            "embedding-empty-state",
            embeddingCopy(
                "No measurement yet. Press the button to compare the two relationship offsets.",
                "No comparison yet. Press the button when you are ready.",
            ),
        ));
    } else {
        const heading = makeNode("div", "embedding-result-heading");
        const stateNode = embeddingResultState(embeddingDemo.relationshipStale);
        stateNode.id = "relationshipResultState";
        heading.append(
            makeNode("h4", "", embeddingCopy("Directional comparison", "Direction comparison")),
            stateNode,
        );
        resultPanel.append(
            heading,
            buildRelationshipDiagram(result, embeddingDemo.relationshipRequest),
            makeNode(
                "p",
                "stage-note embedding-result-note",
                embeddingCopy(
                    "The angle is derived from high-dimensional vector directions. The diagram is not a two-dimensional embedding projection.",
                    "The sketch compares directions. It does not show the complete word map.",
                ),
            ),
        );
    }
    section.append(form, resultPanel);
    queueMicrotask(() => wireEmbeddingDraftInputs(
        [
            "embeddingRelationshipFromA",
            "embeddingRelationshipToA",
            "embeddingRelationshipFromB",
...
function buildStaticAnalogyExperiment() {
    const section = makeNode("section", "embedding-experiment");
    const form = makeNode("form", "embedding-experiment-form");
    form.append(
        makeNode("h3", "", embeddingCopy(
            "Run vector arithmetic",
            "Move through the word map",
        )),
        makeNode("p", "stage-note", embeddingCopy(
            "Create a new vector with A \u2212 B + C, then search for the nearest remaining words.",
            "Start at A, move away from B, then move toward C. Find the closest remaining words.",
        )),
    );
    const presets = makeNode("div", "analogy-presets");
    presets.appendChild(makeNode("span", "preset-label", "Fill an example"));
    EMBEDDING_PRESETS.forEach((preset) => {
        const button = makeNode("button", "preset-button", preset.label);
        button.type = "button";
        button.addEventListener("click", () => {
            captureEmbeddingDraft();
            embeddingDemo.request = { ...preset };
            embeddingDemo.analogyStale = Boolean(state.embeddingAnalogyResult);
            renderEmbeddingStage();
        });
        presets.appendChild(button);
    });
    const equation = makeNode("div", "analogy-equation");
    equation.append(
        inputField("embeddingPositiveA", "Add A", embeddingDemo.request.positiveA),
        makeNode("span", "equation-operator", "\u2212"),
        inputField("embeddingNegative", "Subtract B", embeddingDemo.request.negative),
        makeNode("span", "equation-operator", "+"),
        inputField("embeddingPositiveB", "Add C", embeddingDemo.request.positiveB),
    );
    const run = makeNode(
        "button",
        "primary embedding-run",
        embeddingDemo.loading === "analogy"
            ? "Finding nearest words\u2026"
            : embeddingCopy("Find nearest words", "Find the closest words"),
    );
    run.type = "submit";
    run.disabled = Boolean(embeddingDemo.loading);
    form.append(presets, equation, run);
    form.addEventListener("submit", (event) => {
        event.preventDefault();
        runEmbeddingAnalogy(readAnalogyForm());
    });
    ...
    const resultPanel = makeNode("div", "embedding-result-panel");
    const result = state.embeddingAnalogyResult;
    if (!result) {
        resultPanel.appendChild(makeNode(
            "p",
            "embedding-empty-state",
            embeddingCopy(
                "No search yet. Examples fill the fields only; press the button to calculate.",
                "No search yet. Pick or type an example, then press the button.",
            ),
        ));
    } else {
        const heading = makeNode("div", "embedding-result-heading");
        const stateNode = embeddingResultState(embeddingDemo.analogyStale);
        stateNode.id = "analogyResultState";
        heading.append(
            makeNode("h4", "", result.expression || "A \u2212 B + C"),
            stateNode,
        );
        resultPanel.append(
            heading,
            ...
            makeNode(
                ...
                "stage-note embedding-result-note",
                embeddingCopy(
                    "This result is a nearest-neighbour search from the calculated vector. It does not measure the two relationships above.",
                    "This result finds words near the new map location. It is separate from the direction comparison above.",
                ),
            ),
        );
    }
    section.append(form, resultPanel);
    queueMicrotask(() => wireEmbeddingDraftInputs(
        ["embeddingPositiveA", "embeddingNegative", "embeddingPositiveB"],
        "analogy",
    ));
    return section;
}

function buildStaticVectorDetails() {
    const details = makeNode("details", "embedding-details");
    details.appendChild(makeNode("summary", "", embeddingCopy(
        "Inspect vector coordinates",
        "Inspect word addresses",
    )));
    const body = makeNode("div", "embedding-details-body");
    const previews = {
        ...(state.embeddingRelationshipResult?.vectorPreviews || {}),
        ...(state.embeddingAnalogyResult?.vectorPreviews || {}),
    };
    const words = Object.keys(previews);
    if (words.length === 0) {
        body.appendChild(makeNode(
            "p",
            "stage-note",
            "Run either experiment to inspect the first six coordinates used in its calculation.",
        ));
    } else {
        const list = makeNode("div", "vector-list");
        words.forEach((word) => {
            const values = previews[word] || [];
            const row = makeNode("div", "vector-row");
            row.append(
                makeNode("strong", "vector-word", word),
                makeNode(
                    "code",
                    "vector-values",
                    `[${values.map((value) => Number(value).toFixed(4)).join(", ")}, ...]`,
...
function buildEmbeddingSummary() {
    const summary = makeNode("section", "embedding-workspace-summary");
    summary.append(
        makeNode("h3", "", "What to take away"),
        makeNode(
            "p",
            "",
            embeddingCopy(
                "Similarity compares existing vector directions. Vector arithmetic creates a new vector and searches for nearby items. Static word embeddings assign one vector to each word; transformers compute contextual hidden states.",
                "One experiment compares existing directions. The other creates a new map location. Static word maps stay fixed, while transformers change token locations for each context.",
            ),
        ),
    );
    return summary;
...
function buildStaticEmbeddingWorkspace(stage) {
    const workspace = makeNode("div", "embedding-workspace");
    workspace.append(
        buildStaticRelationshipExperiment(),
        buildStaticAnalogyExperiment(),
        buildStaticVectorDetails(),
        buildEmbeddingSummary(),
    );
    stage.appendChild(workspace);
}

function buildLiveEmbeddingWorkspace(stage) {
    const workspace = makeNode("div", "embedding-workspace");
    const form = makeNode("form", "live-embedding-workspace-form");
    form.append(
        makeNode("h3", "", embeddingCopy(
            "Generate Azure text embeddings",
            "Place four texts on the map",
        )),
        makeNode(
            "p",
            "stage-note",
            embeddingCopy(
                "The model returns one embedding for each complete input. One call supplies both experiments below.",
                "The model gives each complete text one location. One call supplies both comparisons below.",
            ),
        ),
    );
    const fields = makeNode("div", "live-text-grid");
    ...
    const run = makeNode(
        "button",
        "primary embedding-run",
        embeddingDemo.loading === "live" ? "Generating Azure embeddings\u2026" : "Generate Azure embeddings",
    );
    run.type = "submit";
    run.disabled = Boolean(embeddingDemo.loading) || !liveEmbeddingAvailable();
    form.append(fields, run);
    form.addEventListener("submit", (event) => {
        event.preventDefault();
        runLiveEmbeddingCompare(readLiveEmbeddingForm());
    ...
    if (!state.liveEmbeddingResult) {
        workspace.appendChild(makeNode(
            "p",
            "embedding-empty-state live-workspace-empty",
            liveEmbeddingAvailable()
                ? "No model call yet. Generate embeddings when you are ready."
                : "The Azure endpoint or embedding deployment is not configured.",
        ));
    } else {
        const results = makeNode("div", "live-workspace-results");
        const heading = makeNode("div", "embedding-result-heading");
        const stateNode = embeddingResultState(embeddingDemo.liveStale);
        stateNode.id = "liveResultState";
        heading.append(makeNode("h3", "", "Azure embedding results"), stateNode);
        results.appendChild(heading);

        const similarity = makeNode("section", "live-workspace-section");
        similarity.append(
            makeNode("h4", "", embeddingCopy("Pairwise similarity", "Compare existing directions")),
            makeNode("p", "stage-note", embeddingCopy(
                "Cosine similarity and the equivalent angle measure how closely each pair of text vectors aligns.",
                "The score and angle show how closely each pair points the same way.",
            )),
        );
        const cards = makeNode("div", "live-similarity-grid");
        cards.append(
        ...
        similarity.appendChild(cards);

        const arithmetic = makeNode("section", "live-workspace-section");
        const values = liveArithmeticValues();
        arithmetic.append(
            makeNode("h4", "", embeddingCopy("Vector arithmetic experiment", "Create a new map location")),
            makeNode("p", "stage-note", embeddingCopy(
                "The server computes A \u2212 B + C, then compares that vector with Target.",
                "Start at A, move away from B, move toward C, then compare the result with Target.",
            )),
        );
        const arithmeticResult = makeNode("div", "live-arithmetic-result");
        arithmeticResult.append(
            makeNode("strong", "live-experiment-score", values.cosine.toFixed(3)),
            makeNode("span", "live-similarity-angle", `${values.angle.toFixed(1)}\u00B0 equivalent angle`),
        );
        arithmetic.appendChild(arithmeticResult);
        results.append(similarity, arithmetic);
        workspace.appendChild(results);

        const details = makeNode("details", "embedding-details");
        details.appendChild(makeNode("summary", "", "Inspect embedding coordinates"));
        const body = makeNode("div", "embedding-details-body");
        buildLiveVectorStage(body);
        details.appendChild(body);
        workspace.appendChild(details);
    }
    workspace.appendChild(buildEmbeddingSummary());
    stage.appendChild(workspace);
    queueMicrotask(() => wireEmbeddingDraftInputs(
        LIVE_TEXT_ROLES.map(({ key }) => `liveEmbedding${key}`),
        "live",
    ));
}
...
function renderEmbeddingStage() {
    ...
    stage.replaceChildren();
    if (embeddingDemo.mode === "live") {
        buildLiveEmbeddingWorkspace(stage);
    } else {
        buildStaticEmbeddingWorkspace(stage);
    }
...
async function runEmbeddingRelationship(request) {
    captureEmbeddingDraft();
    embeddingDemo.relationshipRequest = { ...request };
    embeddingDemo.loading = "relationship";
    setEmbeddingStatus("Measuring relationship similarity\u2026");
    renderEmbeddingStage();

    try {
        state.embeddingRelationshipResult = await postJson("/api/embeddings/relationship", request);
        embeddingDemo.relationshipStale = false;
        setEmbeddingStatus("");
    } catch (error) {
        setEmbeddingStatus(error.message, true);
    } finally {
        embeddingDemo.loading = null;
        renderEmbeddingStage();
    }
}

async function runEmbeddingAnalogy(request) {
    captureEmbeddingDraft();
    ...
    try {
        state.embeddingAnalogyResult = await postJson("/api/embeddings/nearest-neighbours", request);
        state.embeddingResult = state.embeddingAnalogyResult;
        embeddingDemo.analogyStale = false;
        setEmbeddingStatus("");
    } catch (error) {
        setEmbeddingStatus(error.message, true);
    } finally {
...
async function openEmbeddingExplainer() {
    ...
    embeddingDemo.relationshipRequest = {
        fromA: "man",
        toA: "king",
        fromB: "woman",
        toB: "queen",
    ...
    state.embeddingRelationshipResult = null;
    state.embeddingAnalogyResult = null;
...
function wireEmbeddingExplainer() {
    ...
    el("embeddingTerminologyTechnical").addEventListener("click", () => setEmbeddingTerminology("technical"));
    el("embeddingTerminologyAnalogy").addEventListener("click", () => setEmbeddingTerminology("analogy"));
